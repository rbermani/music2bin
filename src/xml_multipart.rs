...
type MusicalPart = Vec<MusicElement>;
...
pub fn process_xml_multipart(input: PathBuf, output: PathBuf, dump_input: bool) -> Result<()> {
    ...
    let mut part_list = BTreeSet::new();
    for score_part in score_parts {
        part_list.insert(score_part.attribute("id").unwrap());
    }
    println!("Number of parts {}", part_list.len());
    // let mut complete_music: MusicalPart = Vec::new();
